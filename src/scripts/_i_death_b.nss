...
void DoGhostRespawn(object oPC, object oShrine)
{
    ...
    SendDebugMessage("DoGhostRespawn: Shrine Tag: " + sShrine);
    SendDebugMessage("DoGhostRespawn: Original Tag extracted: " + sOriginalTag);
    ...
    SendDebugMessage("DoGhostRespawn: PC moved to original shrine.");

    PayRespawnCost(oPC);
    SetIsGhost(oPC, FALSE);

    SendDebugMessage("DoGhostRespawn: Completed for PC: " + GetName(oPC));
}
...
void DropInventory(object oPC, object oCorpse)
{
    SendDebugMessage("DropInventory: Started for PC: " + GetName(oPC));
    ...
    if (DROP_INVENTORY_ON_DEATH && GetHitDice(oPC) >= LEVEL_PLAYER_DROPS_INVENTORY)
    {
        ...
        while (GetIsObjectValid(oItem))
        {
            if (GetPlotFlag(oItem) == FALSE)
            {
                ...
                DestroyObject(oItem);
                SendDebugMessage("DropInventory: Dropped item: " + GetName(oItem));
            }
    ...
    if (DROP_EQUIPPED_ON_DEATH && GetHitDice(oPC) >= LEVEL_PLAYER_DROPS_INVENTORY)
    {
        ...
        for (nSlot = 0; nSlot < NUM_INVENTORY_SLOTS; nSlot++)
        {
            ...
            if (GetIsObjectValid(oEquippedItem) && GetPlotFlag(oEquippedItem) == FALSE)
            {
                ...
                SendDebugMessage("DropInventory: Dropped equipped item: " + GetName(oEquippedItem));
            }
        }
    }
    ...
    if (DROP_GOLD_ON_DEATH && GetHitDice(oPC) >= LEVEL_PLAYER_DROPS_INVENTORY)
    {
        ...
        SendDebugMessage("DropInventory: Gold amount to drop: " + IntToString(nGoldAmount));
        TakeGoldFromCreature(nGoldAmount, oPC, TRUE);
        GiveGoldToCreature(oCorpse, nGoldAmount);
    }

    SendDebugMessage("DropInventory: Completed for PC: " + GetName(oPC));
}
...
object GetCorpse(object oPC)
{
    ...
    SendDebugMessage("GetCorpse: Generated Corpse Tag: " + sCorpse);
...
object CreateCorpse(object oPC)
{
    SendDebugMessage("CreateCorpse: Started for PC: " + GetName(oPC));

    object oPreviousCorpse = GetCorpse(oPC);
    if (oPreviousCorpse != OBJECT_INVALID)
    {
        DestroyObject(oPreviousCorpse);
        SendDebugMessage("CreateCorpse: Destroyed previous corpse.");
    }
    ...
    SendDebugMessage("CreateCorpse: Created new corpse with tag: " + sCorpse);

    return oCorpse;
}

// Function to handle if a raise/rez is used on a corpse
void RespawnPlayerAtCorpse(object oCorpse)
{
...
void DoDeath(object oPC)
{
    SendDebugMessage("DoDeath: Started for PC: " + GetName(oPC));

    object oCorpse = CreateCorpse(oPC);
    string sCorpseTag = GetTag(oCorpse);
    object oHostileActor = GetLocalObject(oPC, "LAST_HOSTILE");

    SendDebugMessage("DoDeath: Last Hostile Actor: " + GetName(oHostileActor));
    ...
    if (GetIsPC(oHostileActor) == TRUE)
    {
        ...
        SendDebugMessage("DoDeath: PvP Death. Remaining lives: " + IntToString(iRemainingLives));
    ...
    if (iRemainingLives <= 0 && GetIsPC(oHostileActor) == TRUE)
    {
        ...
        SendDebugMessage("DoDeath: Permadeath triggered. PC moved to waypoint.");
    } else
    {
        ...
        DelayCommand(0.2, DelayDestroyObject(oGhostCorpse));
        SendDebugMessage("DoDeath: Ghost respawn executed for PC: " + GetName(oPC));
    }

    SendDebugMessage("DoDeath: Completed for PC: " + GetName(oPC));
}

int GetIsGhost(object oPC)
{
    SendDebugMessage("GetIsGhost: Checking if PC is a ghost: " + GetName(oPC));
    int iGhost = GetCampaignInt(DEATH_DATABASE, GHOST_PLAYER, oPC);
    SendDebugMessage("GetIsGhost: Ghost state is " + IntToString(iGhost));
    return iGhost;
}

void SetIsGhost(object oPC, int iState)
{
    SendDebugMessage("SetIsGhost: Setting ghost state for PC: " + GetName(oPC) + " to " + IntToString(iState));
    if (iState == TRUE)
    {
        ...
        {
            SetPlotFlag(oPC, TRUE);
            SendDebugMessage("SetIsGhost: Plot flag set to TRUE for ghost.");
        }
    }
    else
    {
        ...
        {
            SetPlotFlag(oPC, FALSE);
            SendDebugMessage("SetIsGhost: Plot flag reset to FALSE for non-ghost.");
        }
    }
    SendDebugMessage("SetIsGhost: Completed setting ghost state for PC: " + GetName(oPC));
}

int GetIsGhostArea(object oArea)
{
    SendDebugMessage("GetIsGhostArea: Checking if area is a ghost area: " + GetName(oArea));
    int iGhostArea = GetLocalInt(oArea, GHOST_AREA);
    SendDebugMessage("GetIsGhostArea: Ghost area state is " + IntToString(iGhostArea));
    return iGhostArea;
}

object CreateGhostArea(object oArea)
{
    SendDebugMessage("CreateGhostArea: Creating ghost area from: " + GetName(oArea));
    ...
    SendDebugMessage("CreateGhostArea: Area Tag: " + sTag + ", ResRef: " + sResRef);
    ...
    {
        oNewArea = oAreaCheck;
        SendDebugMessage("CreateGhostArea: Ghost area already exists.");
    }
    else
    {
        ...
        SendDebugMessage("CreateGhostArea: New ghost area created with name: [Death] " + sName);
    }
    return oNewArea;
}
...
{
    SendDebugMessage("SetupGhostArea: Setting up ghost area: " + GetName(oArea));
    ...
    while (GetIsObjectValid(oObject))
    {
            ...
            !GetIsDM(oObject))
        {
            ...
            SendDebugMessage("SetupGhostArea: Destroyed NPC: " + GetName(oObject));
        }
            ...
            GetHasInventory(oObject))
        {
            ...
            SendDebugMessage("SetupGhostArea: Destroyed container contents: " + GetName(oObject));
        }
        oObject = GetNextObjectInArea(oArea);
    }
    SendDebugMessage("SetupGhostArea: Completed setup for ghost area: " + GetName(oArea));
}
...
void PVPSetRemainingLives(object oPC, int iLives)
{
    ...
    SendDebugMessage("PVPSetRemainingLives: Setting PvP lives for PC: " + GetName(oPC) + " to " + IntToString(iLives));
...
int PVPGetRemainingLives(object oPC)
{
    SendDebugMessage("PVPGetRemainingLives: Checking PvP lives for PC: " + GetName(oPC));
    ...
    if (iLivesInitialized == FALSE)
    {
        ...
        SendDebugMessage("PVPGetRemainingLives: PvP lives not initialized. Setting to base: " + IntToString(PVP_LIVES_BASE));
        PVPSetRemainingLives(oPC, iRemainingLives);
    }
    else
    {
        iRemainingLives = GetCampaignInt(DEATH_DATABASE, PVP_LIVES, oPC);
        SendDebugMessage("PVPGetRemainingLives: PvP lives retrieved: " + IntToString(iRemainingLives));
    }
...
void SetPlayerClosure(object oPC, int iState)
{
    SendDebugMessage("SetPlayerClosure: Setting player closure for PC: " + GetName(oPC) + " to " + IntToString(iState));
    SetCampaignInt(DEATH_DATABASE, PC_CLOSURE, iState, oPC);
}

int GetPlayerClosure(object oPC)
{
    SendDebugMessage("GetPlayerClosure: Checking player closure for PC: " + GetName(oPC));
    int iClosure = GetCampaignInt(DEATH_DATABASE, PC_CLOSURE, oPC);
    SendDebugMessage("GetPlayerClosure: Player closure state is " + IntToString(iClosure));
    return iClosure;
}
